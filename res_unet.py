...
from keras.layers import Input, Dropout, ReLU, ELU, LeakyReLU, Concatenate, Activation, Lambda, BatchNormalization, Reshape, Add, Conv1D, PReLU, Activation
...
class ResUNet(object):
    ...
    def down_block(self, feature_map, depth, kernel, stride, drop_rate, first=False, bridge=False):
        ...
        x = Dropout(drop_rate)(x, training = True)
        if not bridge: x = Add()([x,Dropout(drop_rate)(self.Conv(depth, kernel, strides=stride, padding='same', kernel_regularizer=reg)(feature_map), training = True)])
        return x
    ...
    def up_block(self, feature_map, copy_map, depth, kernel, stride, drop_rate):
        ...
        x = self.Conv(depth, kernel, padding='same', kernel_regularizer=reg)(x)
        x = Dropout(drop_rate)(x, training = True)
        x = Add()([x,Dropout(drop_rate)(self.Conv(depth, kernel, padding='same', kernel_regularizer=reg)(x_c), training = True)])
        return x
    ...
    def build_unet(self):
        ...
        for k,s,d,dr in zip(self.kernels,self.strides,self.depth,self.dropout_rates):
            ...
            x = self.down_block(x,d,k,s,dr, first=first, bridge=bridge)
            first = False
...
#         xr = Reshape((64,1))(x)
#         x = Conv1D(32,3,padding='same')(xr)
#         x = Dropout(.1)(x, training = True)
#         x = ELU()(x)
#         x = Conv1D(1,3,padding='same')(x)
#         output_power = Concatenate()([xr,x])
...
#                     optimizer=Adamax(lr=0.002, beta_1=0.9, beta_2=0.999, clipnorm=100.))
    ...
    def like(self, y_true, y_pred):
        ...
        var = K.reshape(y_pred[:,:,:,1],[-1,128,128,1])
    ...
    def plike(self, y_true, y_pred):
        ...
        y_true=pc(K.reshape(y_true,(-1,128,128)))
        mean = K.reshape(y_pred[:,:,0],[-1,64])
        var = K.reshape(y_pred[:,:,1],[-1,64])
        diff_sq = K.square(y_true-mean)
        return K.mean(K.sum(.5*diff_sq*K.exp(-var)+.5*var,axis=np.arange(1, len(diff_sq.shape))))
    ...
    def mse_power(self,y_true, y_pred):
        pc = lambda v: ps.power1D(v)
        power_diff_sq = K.square(pc(K.reshape(y_true,(-1,256,256)))-y_pred)
